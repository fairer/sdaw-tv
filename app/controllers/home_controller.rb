		...
		temp_collection.each do |t|
			...
			if ((t.start_date.strftime("%A") == @day) and (t.start_date.strftime("%H") >= @start_time_hour.to_s) and (t.start_date.strftime("%H") < (@start_time_hour + 4).to_s))
				...
				if ((num_saison == "0") and (num_eps == "0"))
					...
					if (aux != [])
						@collection[temp] = {:hour => t.start_date.hour.to_s + ":" + t.start_date.min.to_s + " ", :name => aux[0].name, :infos => aux[0].genre}
					else
					...
					if (eps != [])
						@collection[temp] = {:hour => t.start_date.hour.to_s + ":" + t.start_date.min.to_s + " ", :name => aux[0].name + " ", :infos => eps[0].season.to_s + " " + eps[0].episode_number.to_s}
					else
